...
void main() {
  ...
  vec3 col = mix(baseColor, waveColor, clamp(f * waveOpacity, 0.0, 1.0));
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef({
    ...
    waveOpacity: new THREE.Uniform(waveOpacity),
  ...
  const prevWaveOpacity = useRef(waveOpacity);
  useFrame(({ clock }) => {
    ...
    if (!prevColor.current.slice(0, 3).every((v, i) => v === waveColor[i])) {
    ...
    if (prevWaveOpacity.current !== waveOpacity) {
      u.waveOpacity.value = waveOpacity;
      prevWaveOpacity.current = waveOpacity;
    }
...
export default function Dither({
  ...
  const normalizedWaveOpacity =
    typeof waveColor[3] === 'number' ? Math.max(0, Math.min(1, waveColor[3])) : 1;
  ...
  return (
    ...
    >
      <DitheredWaves
        ...
        waveColor={waveColor}
        baseColor={baseColor}
        waveOpacity={normalizedWaveOpacity}
        colorNum={colorNum}
        pixelSize={pixelSize}
