...
_api_env        = os.getenv("IPFS_API_URLS", "http://localhost:5001,http://localhost:5003")
IPFS_APIS: list[str] = [a.strip().rstrip("/") for a in _api_env.split(",") if a.strip()]
...
def university_login(body: UniversityLoginRequest):
    ...
    user = UNIVERSITY_USERS.get(username)
    if not user or user["password"] != body.password:
        raise HTTPException(status_code=401, detail="Invalid username or password")

    return {
        "token":    f"university-demo-token-{username}",
        "role":     "university",
        "name":     user["name"],
        "username": username,
...
def fetch_from_ipfs_gateways(cid: str) -> Optional[dict]:
    """Fetches JSON from a gateway first, then falls back to direct Kubo APIs."""
    ...
    for api_base in IPFS_APIS:
        try:
            r = requests.post(
                f"{api_base}/api/v0/cat",
                params={"arg": cid},
                timeout=20,
            )
            if r.status_code == 200:
                print(f"Fetched CID {cid} from Kubo API {api_base}")
                return json.loads(r.text)
        except (requests.exceptions.RequestException, json.JSONDecodeError):
            continue

    print(f"WARNING: Could not fetch CID {cid} from any gateway or Kubo API.")
    return None
...
def build_credential_data(
    *,
    ...
    """
    Builds the canonical bare VC payload used for both manual and CSV staging.
    Keeping one builder avoids shape drift between upload paths.
    """
    canonical_did = canonicalize_did(student_did)
    return {
        "@context":  ["https://www.w3.org/2018/credentials/v1"],
        "type":      ["VerifiableCredential", "UniversityDegreeCredential"],
        "issuer":    {"id": f"did:ethr:{UNIVERSITY_ADDRESS}", "name": university_name},
...
def stage_credential_record(batch_id: str, credential_data: dict, student_did: str) -> dict:
    """
    Pins the bare credential and stages it in-memory for later commit.
    Shared by manual entry and CSV upload so both paths behave identically.
    """
    bare_cid = pin_to_cluster(credential_data, name=f"bare-{batch_id}")
    if not bare_cid:
        raise HTTPException(
            status_code=503,
            detail=(
    ...
    pending_batches[batch_id]["credentials"].append({
        ...
        "student_did":     canonicalize_did(student_did),
        "bare_cid":        bare_cid,
    })

    position = len(pending_batches[batch_id]["credentials"]) - 1
    return {
        "leaf_index":     position,
        "bare_ipfs_cid":  bare_cid,
        "pin_status_url": f"{CLUSTER_API_URL}/pins/{bare_cid}",
    }


def extract_credential_display_fields(full_cred: dict) -> dict:
    """
    Supports both the canonical VC shape and older flat CSV-staged records.
    This keeps already-committed legacy credentials readable.
    """
    subject = full_cred.get("credentialSubject")
    if not isinstance(subject, dict):
        subject = full_cred

    issuer = full_cred.get("issuer", {})
    issuer_name = issuer.get("name") if isinstance(issuer, dict) else None

    return {
        "student_name":    subject.get("studentName") or subject.get("student_name"),
        "university_name": subject.get("universityName") or subject.get("university_name") or issuer_name,
        "degree":          subject.get("degree"),
        "branch":          subject.get("branch"),
        "graduation_year": subject.get("graduationYear") or subject.get("graduation_year"),
        "cgpa":            subject.get("cgpa"),
        "issued_at":       full_cred.get("issuanceDate"),
    }
...
def add_to_batch(request: AddToBatchRequest):
    ...
    """
    credential_data = build_credential_data(
    ...
    staged = stage_credential_record(request.batch_id, credential_data, request.student_did)

    return {
        "success":        True,
        "batch_id":       request.batch_id,
        "leaf_index":     staged["leaf_index"],
        "bare_ipfs_cid":  staged["bare_ipfs_cid"],
        "note":           "Enriched CID (with Merkle proof) will be pinned and stored on-chain at commit time.",
        "pin_status_url": staged["pin_status_url"],
        "message":        f"Added to batch '{request.batch_id}' at position {staged['leaf_index']}. Commit when ready.",
    }
...
def get_student_credentials(student_did: str):
    ...
    for (batch_id, leaf_index, enriched_cid) in pointers:
        ...
        credentials_list.append({
            ...
            "student_name":    display_fields.get("student_name"),
            "university_name": display_fields.get("university_name"),
            "degree":          display_fields.get("degree"),
            "branch":          display_fields.get("branch"),
            "graduation_year": display_fields.get("graduation_year"),
            "cgpa":            display_fields.get("cgpa"),
            "issued_at":       issued_at,
...
async def upload_batch_csv(batch_id: str, file: UploadFile = File(...)):
    ...
    added_count = 0
    for row in reader:
        cgpa_raw = (row.get('cgpa') or '').strip()
        credential_data = build_credential_data(
            student_did=(row.get('student_did') or '').strip(),
